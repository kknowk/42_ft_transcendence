...
export class GameGateway {
  ...
  handleConnection(client: Socket) {
    ...
    const gameRoom = this.gameRooms[gameRoomId];
    const position =
      Object.keys(gameRoom.players).length === 0 ? 'right' : 'left';
  ...
  private async updateGameState(gameRoomId: string) {
    ...
    // 上下の壁に衝突した場合の処理
    if (
      gameRoom.ball.y - gameRoom.ball.radius < 0 ||
      gameRoom.ball.y + gameRoom.ball.radius > 600
    ) {
      gameRoom.ball.dy = -gameRoom.ball.dy;
      if (gameRoom.isLunatic) {
        console.log('Lunatic!!!!!!!!!!!!!!!!!!!!');
        gameRoom.ball.dx = Math.min(
          gameRoom.ball.dx * 1.5 * gameRoom.isLunatic,
          MAX_SPEED,
        );
        gameRoom.ball.dy = Math.min(
          gameRoom.ball.dy * 1.5 * gameRoom.isLunatic,
          MAX_SPEED,
        );
      }
    }
    ...
    Object.values(gameRoom.players).forEach((player) => {
      if (
        (player.position === 'left' &&
          gameRoom.ball.x - gameRoom.ball.radius < 10) ||
        (player.position === 'right' &&
          gameRoom.ball.x + gameRoom.ball.radius > 900 - 10)
      ) {
        // パドルの長さ
        ...
        if (
          ...
          gameRoom.ball.y < player.paddleY + 75
        ) {
          // パドルのセグメントに基づいて角度を計算
    ...
    // 左右の壁に衝突した場合の処理
    if (
      gameRoom.ball.x - gameRoom.ball.radius < 0 ||
      gameRoom.ball.x + gameRoom.ball.radius > 900
    ) {
      Object.values(gameRoom.players).forEach((player) => {
        if (
          gameRoom.ball.x - gameRoom.ball.radius < 0 &&
        ...
        if (player.score <= 0) gameRoom.gameOver = true;
      });
      ...
      if (gameRoom.gameOver) {
        ...
        if (winnerId && loserId) {
          ...
          Object.keys(gameRoom.players).forEach((clientId) => {
            this.server
              .to(clientId)
              .emit('gameOver', { winner: winnerId, loser: loserId });
          });
    ...
    Object.keys(gameRoom.players).forEach((clientId) => {
      const clientBallX =
        gameRoom.players[clientId].position === 'right'
          ? 900 - gameRoom.ball.x
          : gameRoom.ball.x;
